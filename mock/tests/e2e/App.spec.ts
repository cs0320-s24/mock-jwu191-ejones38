...
test("test Load empty csv/improper fileName", async ({ page }) => {
  ...
  await page.getByLabel("Command input").fill("load_file & smallefef.csv & true");
  await page.getByRole("button", { name: "Submitted 1 times" }).click();
...
test("test Load correct load/load switch", async ({ page }) => {
  await page.getByLabel("Login").click();
  await page.getByLabel("Command input").fill("load_file & mockSmall.csv & true");
  await page.getByRole("button", { name: "Submitted 0 times" }).click();

  await page.getByLabel("Command input").fill("mode");
  await page.getByRole("button", { name: "Submitted 1 times" }).click();

  await page .getByLabel("Command input").fill("load_file & income_by_race.csv & true");
  await page.getByRole("button", { name: "Submitted 2 times" }).click();
...
test("view different files", async ({ page }) => {
  await page.getByLabel("Login").click();
  await page
    .getByLabel("Command input")
    .fill("load_file & mockSmall.csv & true");
...
test("search with improper args/no file loaded", async ({ page }) => {
  await page.getByLabel("Login").click();
...
test("search by index/index out of bounds", async ({ page }) => {
  await page.getByLabel("Login").click();
  await page.getByLabel("Command input").fill("load_file & mockSmall2.csv & false");
  await page.getByRole("button", { name: "Submitted 0 times" }).click();
...
test("search by column/(column doesn't exist/no headers)", async ({ page }) => {
  await page.getByLabel("Login").click();
  await page.getByLabel("Command input").fill("load_file & mockSmall2.csv & false");
  await page.getByRole("button", { name: "Submitted 0 times" }).click();
  ...
  expect(output1).toEqual("File has no headers; Cannot do string column search");
  expect(output2).toEqual("NameAgeCountryEmily25Canada");
  expect(output3).toEqual("NameAgeCountry");
  expect(output4).toEqual("Command: search & Nintendo & 44Output: Column could not be found");
});
